                ...
                <div class="conta-card" data-id="${conta.id}">
                    <div class="conta-header">
                        ...
                        <div class="conta-info">
                            <h4>${conta.nome}</h4>
                            <span class="conta-meta">${conta.instituicao || ''} ${conta.instituicao ? '•' : ''} ${labelsTipo[conta.tipo] || conta.tipo}</span>
                    ...
                    <div class="conta-acoes">
                        <button onclick="verMovimentacoesConta('${conta.id}', '${conta.nome}')" class="btn-acao" title="Ver Movimentações" style="color: var(--cor-sucesso);">
                            <i data-lucide="list"></i> Movimentações
                        </button>
                        <button onclick="abrirModalEditarConta('${conta.id}')" class="btn-acao" title="Editar">
                            <i data-lucide="edit-3"></i> Editar
                        </button>
                        <button onclick="inativarConta('${conta.id}', '${conta.nome}')" class="btn-acao" title="Inativar">
                            <i data-lucide="eye-off"></i> Inativar
                        </button>
                        <button onclick="excluirConta('${conta.id}', '${conta.nome}')" class="btn-acao btn-delete" title="Excluir permanentemente">
...
// --- EDITAR CONTA (RF112) ---
window.abrirModalEditarConta = async function (id) {
    try {
...
if (formEditarConta) {
    formEditarConta.addEventListener("submit", async (e) => {
        ...
        try {
            ...
            // Buscar saldo atual para recalcular se mudar o saldo_inicial
...
// --- INATIVAR CONTA (RF113 + RF114: verificar movimentações) ---
window.inativarConta = async function (id, nome) {
    try {
...
// --- EXCLUIR CONTA (RF114: hard delete com verificação de vínculos) ---
window.excluirConta = async function (id, nome) {
    try {
        const [resVendas, resDespesas] = await Promise.all([
            supabase.from('vendas').select('id', { count: 'exact', head: true }).eq('conta_id', id),
            supabase.from('despesas').select('id', { count: 'exact', head: true }).eq('conta_id', id)
        ...
        let mensagem;
        if (totalVinculadas > 0) {
            mensagem = `A conta "${nome}" possui ${totalVinculadas} movimentação(ões) vinculada(s). Ao excluir, o vínculo será removido (os registros de vendas/gastos serão mantidos, mas sem conta vinculada). Deseja continuar?`;
        } else {
...
window.abrirModalTransferencia = async function () {
    ...
    try {
        ...
        const options = '<option value="">Selecione...</option>' +
            (contas || []).map(c => `<option value="${c.id}">${c.nome} (R$ ${parseFloat(c.saldo_atual).toFixed(2)})</option>`).join('');
...
window.verMovimentacoesConta = async function (contaId, contaNome) {
    try {
        const [resVendas, resDespesas] = await Promise.all([
        ...
        if (vendas.length > 0) {
            ...
            vendas.forEach(v => {
                html += `<div style="display:flex; justify-content:space-between; padding: 6px 0; border-bottom: 1px solid var(--borda-cor);">
                    <span style="font-size:0.82rem; color: var(--texto-secundario);">${v.descricao}<br><small>${new Date(v.created_at).toLocaleDateString('pt-BR')} — ${v.status}</small></span>
        ...
        if (despesas.length > 0) {
            ...
            despesas.forEach(g => {
                html += `<div style="display:flex; justify-content:space-between; padding: 6px 0; border-bottom: 1px solid var(--borda-cor);">
                    <span style="font-size:0.82rem; color: var(--texto-secundario);">${g.descricao}<br><small>${new Date(g.data + 'T12:00:00').toLocaleDateString('pt-BR')} — ${g.categoria}</small></span>
